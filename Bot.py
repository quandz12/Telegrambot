...
import string
import asyncio
import os # >>> ĐÃ THÊM THƯ VIỆN NÀY
...
# ... (Các hàm get_global_data, get_user_data, tinh_ty_le giữ nguyên) ...
...
# ... (Các hàm get_main_keyboard, send_welcome_message giữ nguyên) ...
...
# ================== Xử lý /start (Async) ==================
# ... (Các hàm start, button_callback giữ nguyên) ...
...
# ================== CÁC HÀM LỆNH (Async) ==================
# ... (Các hàm sodu, rut, xu_ly_cuoc, duoi_nap, duoi_loc, l_n, taocode, redeem_code, 
# chuyenxu, nap, lichsu_cmd, chinhh_cmd, duyet, hoantra, phien_timer, schedule_next_phien, ket_thuc giữ nguyên) ...
...
# ================== MAIN (ĐÃ ĐIỀU CHỈNH RUN_WEBHOOK) ==================
def main() -> None:
    ...
    # 1. Khởi tạo dữ liệu toàn cục nếu tệp Persistence chưa tồn tại 
    if not application.bot_data:
        application.bot_data.update(GLOBAL_DATA)
    
    # 2. Khai báo Handlers (giữ nguyên)
    ...
    # 4. >>> KHỞI ĐỘNG BOT DƯỚI DẠNG WEBHOOK (Đã Sửa) <<<
    ...
    try:
        ...
        # Biến môi trường này thường do Railway tự động cung cấp
        RAILWAY_APP_DOMAIN = os.environ.get("RAILWAY_STATIC_URL") 
        
        if not RAILWAY_APP_DOMAIN:
            # Fallback (Cần thay bằng tên miền thực của bạn sau khi triển khai)
            print("⚠️ CẢNH BÁO: RAILWAY_STATIC_URL không tồn tại, sử dụng tên miền mẫu.")
            RAILWAY_APP_DOMAIN = "your-app-name.up.railway.app" 
        
        WEBHOOK_URL = f"https://{RAILWAY_APP_DOMAIN}/{WEBHOOK_TOKEN}"

    except Exception as e:
        print(f"Lỗi khi lấy biến môi trường: {e}")
        sys.exit(1) # Dừng nếu không có token/port

    print("👑 BOT GAY GEM ĐANG CHẠY (NÂNG CẤP V20+ ASYNC, PERSISTENCE, WEBHOOK)...")
